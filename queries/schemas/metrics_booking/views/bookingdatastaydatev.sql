CREATE OR REPLACE VIEW `aparium-dataflow.BookingData.BookingDataStayDateV`
AS SELECT
  *,
  CASE property_code
    WHEN 'DENCHM' THEN 63
  ...
  CASE property_code
    ...
    WHEN 'XXXXXX' THEN 'Unknown'
    ELSE NULL
  END AS property_name

FROM `aparium-dataflow.BookingData.BookingData`
WHERE DATE(arrival_date) >= DATE_SUB(CURRENT_DATE(), INTERVAL 1095 DAY)
